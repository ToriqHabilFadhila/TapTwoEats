...
@section('content')
<div style="min-height:100vh; padding:5rem 0 7rem; position:relative;">
    {{-- Decorative blobs --}}
    <div style="position:fixed; top:15%; right:-80px; width:360px; height:360px; background:radial-gradient(circle,rgba(251,146,60,0.15) 0%,transparent 70%); border-radius:50%; pointer-events:none; z-index:0;"></div>
    <div style="position:fixed; bottom:20%; left:-100px; width:400px; height:400px; background:radial-gradient(circle,rgba(79,126,247,0.12) 0%,transparent 70%); border-radius:50%; pointer-events:none; z-index:0;"></div>
    <div style="max-width:900px; margin:0 auto; padding:0 1.5rem; position:relative; z-index:1;">
        ...
        <div class="anim-fade-up" style="text-align:center; margin-bottom:4rem;">
            <div style="display:inline-flex; align-items:center; gap:8px; background:rgba(255,255,255,0.7); backdrop-filter:blur(16px); border:1px solid rgba(255,255,255,0.9); border-radius:99px; padding:8px 20px; margin-bottom:1.5rem; box-shadow:0 4px 16px rgba(79,126,247,0.1);">
                <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#4F7EF7" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" style="display:block; transform:translateY(-0.5px);">
            ...
            <h1 style="font-family:'Lora',serif; font-size:clamp(2.4rem,6vw,4rem); font-weight:600; color:#18243e; margin:0 0 1.2rem; letter-spacing:-0.02em;">
        ...
        {{-- Stats Row --}}
        <div class="anim-fade-up delay-1" style="display:grid; grid-template-columns:repeat(3,1fr); gap:1rem; margin-bottom:2.5rem;">
            @foreach([
                [
            ...
            <div class="glass-card" style="padding:2rem 1rem; text-align:center;">
                <div style="width:52px; height:52px; background:{{ $item['bg'] }}; border-radius:16px; display:flex; align-items:center; justify-content:center; margin:0 auto 1rem;">
                ...
                <div style="font-size:2.2rem; font-weight:800; color:{{ $item['color'] }}; line-height:1; margin-bottom:0.4rem;">
                    {{ $item['num'] }}
                </div>
                <div style="font-size:0.85rem; font-weight:600; color:#7d8fb0;">
                    {{ $item['label'] }}
                </div>
            </div>
            @endforeach
        ...
        {{-- Our Story --}}
        <div class="anim-fade-up delay-2 glass-card" style="padding:2.5rem; margin-bottom:1.5rem; border-radius:24px;">
            ...
            <p style="color:#4a5a7a; line-height:1.8; font-size:0.98rem; margin:0 0 1rem;">
                TapTwoEats was founded in 2024 with a simple mission: to connect food lovers with their favorite restaurants through a seamless digital experience. We believe great food should be accessible to everyone, everywhere.
            </p>
            <p style="color:#4a5a7a; line-height:1.8; font-size:0.98rem; margin:0;">
                Today, we partner with thousands of restaurants and serve millions of happy customers every month, delivering not just food, but moments of joy right to your doorstep.
            </p>
        </div>
        {{-- Values Grid --}}
        <div class="anim-fade-up delay-3" style="display:grid; grid-template-columns:repeat(auto-fit,minmax(200px,1fr)); gap:1rem; margin-bottom:1.5rem;">
            @foreach([
                [
            ...
            <div class="glass-card" style="padding:1.75rem 1.4rem; border-radius:20px;">
                <div style="width:46px;height:46px;background:{{ $item['bg'] }}; border-radius:14px; display:flex; align-items:center; justify-content:center; margin-bottom:1rem;">
        ...
        {{-- Team / CTA --}}
        <div class="anim-fade-up delay-4 glass-card" style="padding:2.5rem; text-align:center; border-radius:24px; background:linear-gradient(135deg,rgba(79,126,247,0.08),rgba(34,211,238,0.06));">
            {{-- Food Icon --}}
