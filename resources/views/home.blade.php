            ...
            <div style="text-align:center; position:relative; z-index:1;">
                ...
                <div class="anim-fade-up delay-4" style="display:flex; justify-content:center; gap:3rem; margin-top:3rem; flex-wrap:wrap;">
                    ...
                    <div style="display:flex; flex-direction:column; align-items:center; gap:6px; text-align:center;">
                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6" class="w-8 h-8 text-blue-500 shrink-0" style="display:block;">
                            {!! $item['icon'] !!}
                        </svg>
                        <div>
                            <div style="font-size:1.6rem; font-weight:800; color:#18243e; line-height:1.1;">
                                {{ $item['num'] }}
                            </div>
                            <div style="font-size:0.82rem; color:#7d8fb0; font-weight:500;">
                                {{ $item['label'] }}
                            </div>
                        </div>
                    </div>
    ...
    <section style="padding:3rem 0;">
        <div style="max-width:1200px; margin:0 auto; padding:0 1.5rem;">
            ...
            <div style="display:grid; grid-template-columns:repeat(auto-fit,minmax(160px,1fr)); gap:1rem;">
                @foreach([
                    ['🍕','Pizza','from-orange-100 to-red-50','#ea580c'],
                    ['🍔','Burgers','from-yellow-100 to-amber-50','#d97706'],
                    ['🍜','Noodles','from-red-100 to-pink-50','#e11d48'],
                    ['🍱','Sushi','from-green-100 to-emerald-50','#059669'],
                    ['🍰','Desserts','from-pink-100 to-rose-50','#db2777'],
                    ['🥗','Salads','from-lime-100 to-green-50','#65a30d'],
                    ['🍗','Chicken','from-orange-100 to-yellow-50','#f59e0b'],
                    ['🧋','Drinks','from-blue-100 to-cyan-50','#0891b2'],
                ] as [$emoji, $name, $grad, $color])
    ...
    <section style="padding:3rem 0 6rem;">
        <div style="max-width:1200px; margin:0 auto; padding:0 1.5rem;">
            <div style="display:flex; justify-content:space-between; align-items:flex-end; margin-bottom:1.8rem;">
                ...
                <a href="{{ route('restaurants') }}" style="display:inline-flex; align-items:center; gap:6px; font-weight:700; font-size:0.9rem; color:#4F7EF7; text-decoration:none; padding:10px 20px; background:rgba(79,126,247,0.08); border-radius:99px; transition:all 0.2s ease;">
                    View All
            ...
            <div style="display:grid; grid-template-columns:repeat(auto-fill,minmax(280px,1fr)); gap:1.25rem;">
                @php
                    ...
                    $cuisines = ['Italian · Pizza · Pasta', 'American · Burgers · Fries', 'Asian · Noodles · Ramen', 'Fusion · BBQ · Grilled', 'Japanese · Sushi · Rolls', 'Mediterranean · Healthy'];
                @endphp
                @for ($i = 0; $i < 6; $i++)
                <div class="glass-card" style="overflow:hidden; border-radius:20px; cursor:pointer;">
                    ...
                    <div style="height:160px; background:{{ $gradients[$i] }}; position:relative; overflow:hidden;">
                        ...
                        <div style="position:absolute; top:12px; left:12px; background:rgba(255,255,255,0.85); backdrop-filter:blur(10px); border-radius:99px; padding:5px 12px; font-size:0.76rem; font-weight:700; color:#18243e; display:flex; align-items:center; gap:6px;">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.6" stroke="currentColor" style="width:14px;height:14px;flex-shrink:0;">
                        ...
                        {{-- Featured badge --}}
                        @if($i === 0 || $i === 2)
                        <div style="position:absolute; top:12px; right:12px; background:rgba(79,126,247,0.9); backdrop-filter:blur(10px); border-radius:99px; padding:5px 12px; font-size:0.72rem; font-weight:700; color:white; letter-spacing:0.04em; text-transform:uppercase;">Featured</div>
                        @endif
                    </div>
                    {{-- Card Body --}}
                    <div style="padding:1.1rem 1.25rem;">
                        <div style="display:flex; justify-content:space-between; align-items:start; margin-bottom:0.4rem;">
                            ...
                            <div style="display:flex; align-items:center; gap:6px; background:rgba(251,191,36,0.12); border-radius:999px; padding:4px 10px;">
                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6" style="width:14px; height:14px; display:block; color:#f59e0b;">
                        ...
                        <div style="display:flex; justify-content:space-between; align-items:center;">
                            ...
                            <a href="#" style="font-size:0.82rem; font-weight:700; color:#4F7EF7; text-decoration:none; padding:6px 14px; background:rgba(79,126,247,0.1); border-radius:99px;">Order →</a>
                        </div>
                    </div>
                </div>
                @endfor
            ...
            <div style="display:grid; grid-template-columns:repeat(auto-fit,minmax(220px,1fr)); gap:1.5rem;">
                @foreach([
                    [
                        ...
                        'desc' => 'Browse hundreds of restaurants near you',
                    ...
                    [
                        ...
                        'desc' => 'Pick from thousands of delicious dishes',
                        'icon' => '<path d="M3 2v7c0 1.1.9 2 2 2h4a2 2 0 0 0 2-2V2"/><path d="M7 2v20"/><path d="M21 15V2a5 5 0 0 0-5 5v6c0 1.1.9 2 2 2h3Zm0 0v7"/></svg>'
                    ],
                    [
                        'step' => '03',
                        'title' => 'Easy Checkout',
                        'desc' => 'Pay securely in seconds',
                    ...
                    [
                        'step' => '04',
                        'title' => 'Fast Delivery',
                        'desc' => 'Get it delivered hot to your door',
                        'icon' => '<path stroke-linecap="round" stroke-linejoin="round" d="M8.25 18.75a1.5 1.5 0 0 1-3 0m3 0a1.5 1.5 0 0 0-3 0m3 0h6m-9 0H3.375a1.125 1.125 0 0 1-1.125-1.125V14.25m17.25 4.5a1.5 1.5 0 0 1-3 0m3 0a1.5 1.5 0 0 0-3 0m3 0h1.125c.621 0 1.129-.504 1.09-1.124a17.902 17.902 0 0 0-3.213-9.193 2.056 2.056 0 0 0-1.58-.86H14.25M16.5 18.75h-2.25m0-11.177v-.958c0-.568-.422-1.048-.987-1.106a48.554 48.554 0 0 0-10.026 0 1.106 1.106 0 0 0-.987 1.106v7.635m12-6.677v6.677m0 4.5v-4.5m0 0h-12" />'
                    ],
                ] as $item)
                <div style="display:flex; gap:16px; align-items:center;">
                    ...
                    {{-- Text --}}
                    <div>
                        <div style="font-size:0.75rem; font-weight:700; color:#4F7EF7;">
                            STEP {{ $item['step'] }}
                        </div>
                        <div style="font-size:1.1rem; font-weight:800; color:#18243e;">
                            {{ $item['title'] }}
                        </div>
                        <div style="font-size:0.9rem; color:#7d8fb0;">
                            {{ $item['desc'] }}
                        </div>
                    </div>
                </div>
