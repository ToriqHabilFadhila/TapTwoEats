...
<div style="min-height:100vh;padding:2.5rem 0 6rem;">
    <div style="max-width:1300px;margin:0 auto;padding:0 1.5rem;">
        ...
        <div class="anim-fade-up" style="margin-bottom:2.5rem;">
            ...
            <h1 style="font-family:'Lora',serif;font-size:clamp(2rem,4vw,3rem);font-weight:600;color:#18243e;margin:0 0 0.6rem;">Our <span class="text-grad-blue">Menu</span></h1>
            <p style="color:#7d8fb0;font-size:0.98rem;margin:0;">Explore our delicious food selection</p>
        </div>
        {{-- Filter Bar --}}
        <div class="anim-fade-up delay-1" style="display:flex;flex-wrap:wrap;gap:0.6rem;margin-bottom:2.5rem;">
            @foreach(['All', 'Pizza', 'Burgers', 'Noodles', 'Desserts', 'Drinks', 'Salads', 'Chicken'] as $i => $cat)
            <button
                onclick="setFilter(this, '{{ strtolower($cat) }}')"
                style="padding:10px 22px;border-radius:99px;font-family:'Bricolage Grotesque',sans-serif;font-weight:600;font-size:0.88rem;cursor:pointer;transition:all 0.2s ease;border:1.5px solid {{ $i === 0 ? '#4F7EF7' : 'rgba(79,126,247,0.18)' }};background:{{ $i === 0 ? 'linear-gradient(135deg,#4F7EF7,#22D3EE)' : 'rgba(255,255,255,0.65)' }};color:{{ $i === 0 ? '#fff' : '#3d4f76' }};backdrop-filter:blur(12px);"
            >
                {{ $cat }}
            </button>
            @endforeach
        ...
        {{-- Menu Grid --}}
        <div class="anim-fade-up delay-2" style="display:grid;grid-template-columns:repeat(auto-fill,minmax(240px,1fr));gap:1.25rem;">
            @php
            $gradients = [
            ...
            <div class="glass-card" style="overflow:hidden;border-radius:20px;cursor:pointer;">
                ...
                <div style="height:160px;background:{{ $gradients[$i] }};position:relative;overflow:hidden;">
                    <div style="position:absolute;inset:0;display:flex;align-items:center;justify-content:center;font-size:4rem;">{{ $emojis[$i] }}</div>
                    {{-- Wishlist --}}
                    <button style="position:absolute;top:10px;right:10px;background:rgba(255,255,255,0.85);backdrop-filter:blur(10px);border:none;border-radius:99px;width:34px;height:34px;display:flex;align-items:center;justify-content:center;cursor:pointer;transition:all 0.2s;" onclick="this.style.background='rgba(254,202,202,0.95)'">
                    ...
                    {{-- Rating --}}
                    <div style="position:absolute;bottom:10px;left:10px;background:rgba(255,255,255,0.85);backdrop-filter:blur(10px);border-radius:99px;padding:4px 10px;font-size:0.76rem;font-weight:700;color:#18243e;">⭐ 4.{{ 5 + $i % 3 }}</div>
                </div>
                {{-- Card Body --}}
                <div style="padding:1rem 1.1rem 1.1rem;">
                    <h3 style="font-weight:700;font-size:0.98rem;color:#18243e;margin:0 0 0.3rem;">{{ $names[$i] }}</h3>
                    <p style="font-size:0.8rem;color:#7d8fb0;margin:0 0 0.85rem;">Delicious & freshly prepared</p>
                    <div style="display:flex;justify-content:space-between;align-items:center;">
                        <span style="font-size:1.05rem;font-weight:800;background:linear-gradient(135deg,#4F7EF7,#22D3EE);-webkit-background-clip:text;background-clip:text;color:transparent;">Rp 50.000</span>
                        <button class="btn-primary" style="padding:8px 18px;border-radius:10px;font-size:0.82rem;">
                            + Add
                        </button>
                    </div>
                </div>
