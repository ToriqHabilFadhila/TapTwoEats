...
<div style="min-height:100vh;padding:2.5rem 0 6rem;">
    <div style="max-width:1300px;margin:0 auto;padding:0 1.5rem;">
        ...
        <div class="anim-fade-up delay-2" style="background:rgba(255,255,255,0.6);backdrop-filter:blur(20px);border:1px solid rgba(255,255,255,0.9);border-radius:18px;padding:10px;display:inline-flex;flex-wrap:wrap;gap:6px;margin-bottom:2.5rem;box-shadow:0 4px 24px rgba(79,126,247,0.08);">
            @php
            $tabs = [
                ['route' => 'menu.all', 'label' => 'All', 'emoji' => '🍽️', 'active' => true],
                ['route' => 'menu.popular', 'label' => 'Popular', 'emoji' => '🔥', 'active' => false],
                ['route' => 'menu.new', 'label' => 'New Arrivals', 'emoji' => '✨', 'active' => false],
                ['route' => 'menu.deals', 'label' => 'Special Deals', 'emoji' => '🎁', 'active' => false],
            ];
            @endphp
            @foreach($tabs as $tab)
            <a href="{{ route($tab['route']) }}" style="padding:10px 20px;border-radius:12px;font-weight:700;font-size:0.88rem;text-decoration:none;transition:all 0.2s;display:flex;align-items:center;gap:6px; {{ $tab['active'] ? 'background:linear-gradient(135deg,#4F7EF7,#22D3EE);color:#fff;box-shadow:0 4px 14px rgba(79,126,247,0.3);' : 'color:#3d4f76;background:transparent;' }}">
                <span>{{ $tab['emoji'] }}</span> {{ $tab['label'] }}
            </a>
            @endforeach
        </div>
        {{-- Search + Sort Bar --}}
        <div class="anim-fade-up delay-3" style="display:flex;gap:1rem;margin-bottom:2rem;flex-wrap:wrap;">
            <div style="flex:1;min-width:260px;position:relative;">
                ...
                <input type="text" placeholder="Search dishes..." class="input-glass" style="padding-left:44px;">
            </div>
        ...
        <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(240px,1fr));gap:1.25rem;">
            ...
            <div class="glass-card anim-fade-up" style="overflow:hidden;border-radius:20px;cursor:pointer;animation-delay:{{ $i * 0.04 }}s;">
                <div style="height:155px;background:{{ $gradients[$i] }};position:relative;overflow:hidden;">
                    <div style="position:absolute;inset:0;display:flex;align-items:center;justify-content:center;font-size:3.8rem;transition:transform 0.3s;" class="card-emoji">{{ $emojis[$i] }}</div>
